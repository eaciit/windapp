...
func (d *GenScadaPowerCurvePlus) GeneratePlusAdj(base *BaseController) {
	d.Generate(base, "ADJ", "rpt_scadapowercurve_genonly")
}

func (d *GenScadaPowerCurvePlus) GeneratePlusAvg(base *BaseController) {
	d.Generate(base, "AVG", "rpt_scadapowercurve_genonly_avg")
}
...
func (d *GenScadaPowerCurvePlus) Generate(base *BaseController, dataType string, tblName string) {
	if base != nil {
		...
		for _, dataTurbine := range dataTurbines {
			...
			pipe := []tk.M{tk.M{}.Set("$match", tk.M{}.Set("dateinfo.dateid", tDateId).Set("turbine", tTurbine).Set("power", tk.M{}.Set("$gte", 0)).Set("oktime", 600)), tk.M{}.Set("$group", tk.M{}.Set("_id", fieldWs).Set("power", tk.M{}.Set("$sum", "$power")).Set("total", tk.M{}.Set("$sum", 1)))}
			//tk.Printf("#%v\n", pipe)
