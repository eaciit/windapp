...
var (
	stablename = "ScadaDataHFD"
	dtablename = "ScadaDataHFD"
)
...
func main() {
	...
	stime := time.Date(2017, 1, 14, 0, 0, 0, 0, time.UTC)

	csr, _ := _conn.NewQuery().Select().From(stablename).
		Where(dbox.Lt("timestamp", stime)).
		Order("-timestamp").
		Cursor(nil)
	defer csr.Close()
	...
	sdata := make(chan *ScadaDataHFD, count)
...
func workersave(wi int, jobs <-chan *ScadaDataHFD, result chan<- int) {
	...
	trx := new(ScadaDataHFD)
	for trx = range jobs {

		trx.Fast_WindSpeed_Bin = tk.RoundingAuto64(trx.Fast_WindSpeed_ms, 0)
