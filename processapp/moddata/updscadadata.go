...
var (
	stablename = "ScadaData"
	dtablename = "ScadaData-1"
)
...
func main() {
	...
	stablename = new(ScadaData).TableName()
	dtablename = tk.Sprintf("%s-1", new(ScadaData).TableName())

	csr, _ := _conn.NewQuery().Select().From(stablename).Cursor(nil)
	defer csr.Close()

	count := csr.Count()
	sresult := make(chan int, count)
	sdata := make(chan *ScadaData, count)
	for i := 0; i < 5; i++ {
...
func workersave(wi int, jobs <-chan *ScadaData, result chan<- int) {
