...
vm.currentMenu('X/Y Analysis 3D');
vm.currentTitle('X/Y Analysis 3D');
vm.breadcrumb([{
    title: "Analysis Tool Box",
    href: '#'
},{
    title: 'X/Y Analysis 3D',
    href: viewModel.appName + 'page/xyanalysis3d'
}]);
...
page.GenerateChart3D = function(dataSource) {
    var data = null;
    var graph = null;
    ...
    var ylist = {};
    var zlist = {};
    var axisMax = ["xmax", "ymax", "zmax"];
    var axisMin = ["xmin", "ymin", "zmin"];
    var axisScale = [];

    // var myData = dataSource.data[0].detail.sort(Comparator)
    $.each(dataSource.data[0].detail, function(idx, val) {
        if (!(val[0] == null || val[1] == null || val[2] == null )) {
            xlist[val[0].toFixed(1)] = val[0]
            ylist[val[1].toFixed(1)] = val[1]
            zlist[val[0].toFixed(1).toString() + "_" + val[1].toFixed(1).toString()] = val[2]
            
            for (var x = 0; x < 3; x++) {
                if (axisScale[axisMax[x]] == null || axisScale[axisMax[x]] <= val[x]) {
                    axisScale[axisMax[x]] = val[x]
                }

                if (axisScale[axisMin[x]] == null || axisScale[axisMin[x]] >= val[x]) {
                    axisScale[axisMin[x]] = val[x]
                }                
            }
        }
    })

    $.each(xlist, function(idx, valx) {
        $.each(ylist, function(idy, valy) {
            var zinsert = zlist[idx + "_" + idy]
            if ( zinsert == null) {
                zinsert = 0
            }
            // var value = page.customVis(val,_val);
            data.add({id:counter++,x:valx,y:valy,z:zinsert,style:zinsert});
        })
    })
    ...
    var options = {
      ...
      verticalRatio: 0.5,
      xMin : Math.floor(axisScale['xmin']),
      xMax : axisScale['xmax'],
      xStep : Math.round(( axisScale['xmax'] - axisScale['xmin'] )/5),
      yMin : Math.floor(axisScale['ymin']),
      yMax : axisScale['ymax'],
      yStep : Math.round(( axisScale['ymax'] - axisScale['ymin'] )/5),
      zMin : Math.floor(axisScale['zmin']),
      zMax : axisScale['zmax'],
      zStep : Math.round(( axisScale['zmax'] - axisScale['zmin'] )/5),
    };

    // Instantiate our graph object.
    console.log(data)
    console.log(options)
    var container = document.getElementById('chartxyAnalysis');
    graph = new vis.Graph3d(container, data, options);
}

page.customVis = function(x, y) {
